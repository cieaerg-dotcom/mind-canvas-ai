...
# 🚀 新增/修改：加入了 chat_model 參數，讓摘要功能也使用選擇的模型
def update_canvas_summary(client, history, chat_model="gemini-3-flash-preview"):
    summary_prompt = f"根據對話更新目前畫面構思狀態，回傳 JSON (主體, 環境, 光影, 風格)。對話：{history}"
    try:
        res = client.models.generate_content(model=chat_model, contents=summary_prompt)
...
if "last_api_key" not in st.session_state: st.session_state.last_api_key = ""
...
with st.sidebar:
    ...
    # 🚀 新增：動態模型選擇區塊
    st.header("🧠 模型設定")
    target_chat_models = ["gemini-3-flash-preview", "gemini-3-pro-preview", "gemini-2.5-flash", "gemini-2.5-pro"]
    target_img_models = ["imagen-4.0-generate-001", "imagen-3.0-generate-001"]

    def format_model_option(m):
        """根據 API 權限動態改變顯示標籤 (模擬反灰效果)"""
        if not st.session_state.api_key_valid:
            return f"🔒 {m} (需金鑰)"
        if m in st.session_state.available_models:
            return f"✅ {m}"
        return f"❌ {m} (無權限)"

    selected_chat_model = st.selectbox("對話與構圖模型：", target_chat_models, format_func=format_model_option)
    selected_img_model = st.selectbox("最終出圖模型：", target_img_models, format_func=format_model_option)

    # 判斷使用者選擇的模型是否有權限
    chat_usable = st.session_state.api_key_valid and (selected_chat_model in st.session_state.available_models)
    img_usable = st.session_state.api_key_valid and (selected_img_model in st.session_state.available_models)

    if st.session_state.api_key_valid and not chat_usable:
        st.warning("⚠️ 目前選取的對話模型無權限，可能導致報錯。")
    ...
    generate_btn = st.button("✨ 最終具現化", type="primary", use_container_width=True)
...
with col_canvas:
    ...
    st.divider()
    st.markdown(f"#### ✍️ 你的塗鴉板 ({device_type} {orientation if device_type!='電腦' else '橫式'})")
        ...
        with st.chat_message("assistant"):
            try:
                ...
                for m in st.session_state.messages[:-1]:
                    ...
                    clean_txt = m["content"].replace("【已傳送塗鴉板畫面】", "我畫了一張草圖。")
                    full_contents.append({"role": role, "parts": [{"text": clean_txt}]})
                full_contents.append({"role": "user", "parts": current_parts})
                
                # 🚀 變更：對話使用側邊欄選擇的對話模型
                resp = client.models.generate_content(
                    model=selected_chat_model, contents=full_contents,
                    config=types.GenerateContentConfig(system_instruction=st.session_state.persona, temperature=0.7)
                )
                
                ai_reply = clean_ai_message(resp.text)
                if ai_reply:
...
if api_key:
    # 阻擋機制：確保模型可用才允許對話
    if prompt:
        if chat_usable: send_message_to_ai(client, prompt, include_canvas=False)
        else: st.error("❌ 所選對話模型無權限，無法發送訊息。")
        
    if send_drawing_btn:
        if chat_usable: send_message_to_ai(client, "", include_canvas=True)
        else: st.error("❌ 所選對話模型無權限，無法傳送塗鴉。")
        
    if draw_sketch_btn:
        if not chat_usable:
            st.error("❌ 畫圖示範需要對話模型權限，請更換模型！")
        else:
    ...
    if generate_btn:
        if not img_usable:
            st.error("❌ 您選擇的出圖模型無權限，請更換模型！")
        else:
            with st.spinner("✨ 具現化中..."):
                try:
                    ...
                    # 🚀 變更：使用側邊欄選擇的出圖模型
                    img_res = client.models.generate_images(model=selected_img_model, 
